            ...
            // Kademe tablosu
            int kademeler[] = {155, 180, 205, 230, 255};
...
// --- START ---
            ...
            if (state == FINISHED) {
             ...
             // İlk hız 1. Kademe
              pwmValue = kademeler[0];
              motor->setBrightness(pwmValue);
           
              heaterOn = true;
              heater->setBrightness(128);   // START ile otomatik çalışsın
         ...
         void onDevirEksi() {
           if (state == RUNNING) {
             ...
             if (mevcutKademe > 0) {
               ...
               pwmValue = kademeler[mevcutKademe - 1];
               motor->setBrightness(pwmValue); 
               ...
               lcd.print(mevcutKademe);
               lcd.print(". Kademe");
               delay(500);
             } 
             else if (mevcutKademe == 0) {
               // Artık tamamen kapat
               pwmValue = 0;
               motor->setBrightness(pwmValue); 
...
// --- LCD GÖRÜNTÜLEME ---
          void showRunning(unsigned long seconds) {
            ...
            // Ekrana yaz
            if (kademeIndex == -1 || pwmValue == 0) {
              lcd.print("KAPALI   ");
            } else {
              lcd.print(kademeIndex + 1);
              lcd.print(". Kademe ");
            }
