...
// ===== PWM ARTI =====
          void onDevirArti() {
            if (state == RUNNING) {
              // Eğer şu an 200 ise bir sonraki 5. kademe 255 olacak
              if (pwmValue < 200) {
                pwmValue += 50;        // 0,50,100,150,200
              } 
              else if (pwmValue == 200) {
                pwmValue = 255;        // 5. kademe özel olarak 255
              } 
              // eğer zaten 255 ise değişiklik yok
          
              analogWrite(PWM_OUT, pwmValue);
          
              int kademe = (pwmValue == 255) ? 5 : (pwmValue / 50);
              // burada istersen LCD güncellemesi yapabilirsin
            }
          }
...
// ===== PWM EKSI =====
          void onDevirEksi() {
            if (state == RUNNING) {
              // Eğer şu an 255 ise bir önceki kademe 200 olacak
              if (pwmValue == 255) {
                pwmValue = 200;
              } 
              else if (pwmValue >= 50) {
                pwmValue -= 50;       // 250 değil; 200,150,...,50,0
              } 
              // eğer zaten 0 ise değişiklik yok
